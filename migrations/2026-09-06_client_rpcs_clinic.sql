-- Carry clinic_id through the client write RPCs.
--
-- Both functions had accumulated two overloads (with and without
-- p_patient_phone). Adding a parameter would make a third, and PostgREST
-- cannot resolve a named-argument call that matches more than one candidate,
-- so the stale overloads are dropped and a single signature is left standing.
-- The previous definitions are archived in
-- archive/rpc-backup/2026-09-06_client_rpcs_before_clinic.sql.

BEGIN;

DROP FUNCTION IF EXISTS public.insert_client_multi_category(
  uuid, text, text, text[], date, integer, uuid, text[], text, text, text, integer);
DROP FUNCTION IF EXISTS public.insert_client_multi_category(
  uuid, text, text, text[], date, integer, uuid, text[], text, text, text, integer, text);

DROP FUNCTION IF EXISTS public.update_client_group(
  uuid, text, text, text[], date, integer, text[], text, text, text, integer);
DROP FUNCTION IF EXISTS public.update_client_group(
  uuid, text, text, text[], date, integer, text[], text, text, text, integer, text);

CREATE FUNCTION public.insert_client_multi_category(
    p_lab_id uuid,
    p_patient_name text,
    p_notes text,
    p_categories text[],
...
CREATE FUNCTION public.update_client_group(
